...
export default function APIEndpoints(app) {
  ...
  app.get("/gitsongs", async (req, res) => {
    try {
      const response = await fetch(
        `https://api.github.com/repos/anuplohar001/BeatBox/contents/songs/`,
        {
          headers: {
  ...
  app.get("/gitfolder/:folder", async (req, res) => {
    ...
    try {
      const response = await fetch(
        `https://api.github.com/repos/anuplohar001/BeatBox/contents/songs/${folder}`,
        {
          headers: {
            Authorization: `token ${process.env.GITHUB_TOKEN}`,
          },
  ...
  app.get("/messages", async (req, res) => {
    ...
    try {
      ...
      else return res.status(200).send({ message: "No messages" });
    } catch (error) {
      return res.status(500).send({ message: message });
    }
  });
  ...
  app.get("/oldStory", async (req, res) => {
    try {
      ...
      if (stories) return res.status(200).send(stories);
  ...
  app.get("/likes/:postid/:userid", async (req, res) => {
    ...
    try {
      ...
      if (posts.length) return res.status(200).send({ isLike: true });
  ...
  app.put("/postComment", async (req, res) => {
    ...
    try {
      const prompt = await Prompt.findOneAndUpdate(
        { _id: postid },
        { $push: { comment: { content: comment, userid: padmin } } }
      );
  ...
  app.put("/deleteComment", async (req, res) => {
    ...
    try {
      const post = await Prompt.findOneAndUpdate(
